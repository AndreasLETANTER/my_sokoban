...
{
    if (array[i][j] == 'X' && ((array[i + 1][j] == 'X' || array[i + 1]
    [j] == '#')) && (array[i][j + 1] == 'X' || array[i][j + 1] == '#')) {
        nb_objects--;
        return (nb_objects);
    } else if (array[i][j] == 'X' && ((array[i - 1][j] == 'X' || array[i - 1]
    [j] == '#')) && (array[i][j - 1] == 'X' || array[i][j - 1] == '#')) {
        nb_objects--;
        return (nb_objects);
    }
    if (array[i][j] == 'X' && ((array[i][j + 1] == 'X' || array[i]
    [j + 1] == '#')) && (array[i - 1][j] == 'X' || array[i - 1][j] == '#')) {
        nb_objects--;
        return (nb_objects);
    } else if (array[i][j] == 'X' && ((array[i][j - 1] == 'X' || array[i]
    [j - 1] == '#')) && (array[i + 1][j] == 'X' || array[i + 1][j] == '#')) {
        nb_objects--;
        return (nb_objects);
    }
